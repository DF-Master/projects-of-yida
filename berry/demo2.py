...
def parse_args():
    ...
    parser.add_argument(
        "--image_output_folder",
        dest="img_out",
        help="choose image output folder",
        default="images_out",
    )
...
# save a new recogize image
    ...
    # try:
    # consider if keep_boxes has no candidates
    if keep_boxes.shape[0] == 0 or len(keep_boxes) < MIN_BOXES:
        keep_boxes = torch.argsort(max_conf, descending=True)[:MIN_BOXES]
    elif len(keep_boxes) > MAX_BOXES:
        keep_boxes = torch.argsort(max_conf, descending=True)[:MAX_BOXES]
    # except TypeError:
    #     ic("Notice: no box detected")
    ...
    for i in range(len(keep_boxes)):
        ...
        if cls in ['man','shirt','cat','policeman','young man','dog', 'bird', 'boy','woman','girl','dogs',"legs","leg","wrist","face","head"]:
            here_is_animal = True
    ...
    save_dir = os.path.join(args.img_out, args.image_file.rsplit("/")[-1])
    save_dir_origin = os.path.join(args.img_out + "Animals/", "origin_"+args.image_file.rsplit("/")[-1])
    ic(save_dir)
    # ic(os.path.join(args.img_out))
    # exit()
    plt.savefig(save_dir, dpi=500)
    
    if here_is_animal == True:
        shutil.copyfile(im_file,save_dir_origin)


    # plt.savefig(os.path.join(args.img_out, args.image_file), dpi=500)
