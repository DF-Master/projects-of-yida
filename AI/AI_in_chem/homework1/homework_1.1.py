...
train_data = pd.read_table('./AI/AI_in_chem/train.dat', sep=" ",
                           header=None).values.tolist()
test_data = pd.read_table('./AI/AI_in_chem/test.dat', sep=" ",
                          header=None).values.tolist()  # type - list
...
train_X = poly_features.fit_transform(x.reshape(-1,
                                                1))  #reshape (-1,1)转换成一列，为np功能
test_X = poly_features.fit_transform(X_test.reshape(
    -1, 1))  #reshape (-1,1)转换成一列，为np功能
...
for lam in np.logspace(-6, 6, 100):
    ...
    train_model = linear_model.Ridge(
        alpha=lambda_, fit_intercept=True)  # Create a ridge regression model
...
for i in range(len(list_collect_0) - 3):
    plt.plot(np.log(list_collect_0[0]),
             list_collect_0[i + 2],
             label='C' + str(i))
