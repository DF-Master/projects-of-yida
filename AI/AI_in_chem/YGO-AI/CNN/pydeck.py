...
def getText(cardID, dataType):
    ...
    """
    global lookup
    lookup.execute('SELECT ' + dataType + ' FROM texts WHERE id = ' +
                   str(cardID))
...
def getData(cardID, dataType):
    ...
    """
    global lookup
    lookup.execute('SELECT ' + dataType + ' FROM datas WHERE id = ' +
                   str(cardID))
    return lookup.fetchone()[0]
...
class Card:
    def __init__(self, c):
        ...
        self.attribute = self.getAttributeFromID(
            getData(self.cardID, 'attribute'))
...
class Deck:
    ...
    def add(self, card):
        ...
        if self.decktype == "extra":
            s = card.typeString
            if ("Xyz" in s or "Synchro" in s
                    or "Fusion" in s) and len(self.cardList) <= self.limit:
